...
function load_region(){
    ...
    // var tabs = {'region':'Overview','network':'Network'},
		...
		// countyCodes = {'baja california':42017,'imperial':34005,'san diego':34007};
	
	var stat_data = ['IntRouteMi', 'NatHwyMile', 'NHS', 'interch', 'freightRai', 'yards', 'portTermin', 'ShipCalls', 'centers', 'centerEmp', 'truckStop'];

	var county_desc = { 
		'baja california' : 'San Diego is a city on the Pacific coast of California known for its beaches, parks and warm climate. Immense Balboa Park is the site of the renowned San Diego Zoo, as well as numerous art galleries, artist studios, museums and gardens. A deep harbor is home to a large active naval fleet, with the USS Midway, an aircraft-carrier-turned-museum, open to the public.',
	...
	function buildCountyMap(){
		...
		if ($('#c-region-map').length === 0) {
			...
			d3.json('data/d3/Vicinity_20171109_v5.geojson', function(county5k) {
    ...
    function toTitleCaseBaja(str) {
        if($('county_outlines') === 'baja california norte')
		return str.replace(/(?:^|\s)\w/g, function(match) {
	...
	function region_navigation(hash_elem){
		...
		$('.c-region-tab').hide();
		$('#c-'+target).show();
        $('#c-name').html(toTitleCase(county)+' County');
        $('#c-name').html(toTitleCaseBaja(county));
