$(document).ready(function () {
    ...
    d3.csv('data/d3/landport_diagram.csv', function(port_points) {
        d3.csv('data/d3/landport_activity.csv', function(port_data) {
            // d3.json("data/d3/river_diagram.js", function(json) {
            // d3.json("data/d3/Border.geojson", function(json) {
                d3.json("data/d3/ocean_border_merge.json", function(json) {
                // d3.csv("data/d3/river_labels.csv", function(river_labels){
                    d3.json("data/d3/Landport_Indicators.json", function(river_labels){
                    ...
                    port_svg.append('g')
                        ...
                        .attr('id', 'ports');
                    // draw river outline
                    var dia_projection = d3.geo.mercator().scale(1).translate([0, 0]).precision(0).rotate([0, 0, 0]);

                    var river_path = d3.geo.path().projection(dia_projection);
                    var bounds = river_path.bounds(json);

                    var scale = 7.12 / Math.max((bounds[1][0] - bounds[0][0]) / (port_dia_width), ((bounds[1][1] - bounds[0][1]) / port_dia_height));

                    var transl = [(port_dia_width - 2450 - scale * (bounds[1][0] + bounds[0][0])) / 2, (port_dia_height + 750 - scale * (bounds[1][1] + bounds[0][1])) / 2];
                    ...
                    function sizePorts(count_year) {
                        ...
                        d3.selectAll('.port circle')
                            ...
                            .on('mouseout', function(e) {
                                ...
                                $('#mi-vessel-count').html(numeral(tc).format('0,0'));   
                                $('#mi-vessel-percent').html((totalCallChange).toFixed(0) + '%'); 
                                $('#mi-vessel-title').html('All Ports of Entry');
                                if (totalCallChange > 5) {
                                    $('#mi-vessel-icon').html(ind_up);
                                } else if (totalCallChange < -5) {
                                    $('#mi-vessel-icon').html(ind_down);
                    ...
                    // ****************************************
                    // load land indicators trade chart data and build chart

                    d3.csv("data/d3/landportIndicators.csv", function(data) {
                        var labelVar = 'year';

                        var l = new Date();
                        var thisYear = l.getFullYear();

                        var varNames = ['swt_export', 'swt_import'];
                        ...
                        for (var i = 2006; i <= maxDataYear; i++) {
                            $('#landport-year-dropdown').append('<li><input type="radio" id="m' + i + '" class="miYear" name="miYear" value="' + i + '"><label for="m' + i + '">' + i + '</label></li>');
                        }
                        ...
                        completeData.map(function(d) {
                            if (d.year > 2005) {
                                varNames.map(function(name) {     //The map() method is used to apply a function on every element in an array.
                        ...
                        dataWindow.append('div')
                            .html('<div class="mi-dw-title">Trade for <span class="mi-dw-year">2018</span></div>' +
                                '<div class=""><div class="mi-data-swatch dkblue-bg"></div> ' +
                                // 'Domestic: <span class="right_align"><span class="mi-dw-domestic">xxx</span> M tons</span></div><div class=""><div class="mi-data-swatch blue-bg"></div>' +
                                'Imports: $<span class="left_align"><span class="mi-dw-import">xxx</span> B</span></div><div class=""><div class="mi-data-swatch ltblue-bg"></div>' +
                                'Exports: $<span class="left_align"><span class="mi-dw-export">xxx</span> B</span></div><div class="mi-data-swatch orange-bg"></div>' +
                                'Crossings: <span class="left_align"><span class="mi-dw-containers">xxx</span> trucks</span></div>');
                        ...
                        function mousemove() {
                            ...
                            circles.transition()
                                ...
                                .attr('transform', function(d) {
                                    ...
                                    $('.mi-dw-import').html((imp_val * 0.000000001).toFixed(1));
                                    $('.mi-dw-export').html((exp_val * 0.000000001).toFixed(1));
                            ...
                            contCircles.transition()
                                .duration(50)
                                .attr('transform', function(d) {
                                    $('.mi-dw-containers').html((containers[c[1]].value * 1).toFixed(0));
                        ...
                        updateTradePanel = function(mYear) {
                            ...
                            if (data[yr].port_rank !== "") {
                                ...
                                var portRank = data[yr].port_rank,
                                    ...
                                    totalTrade = parseInt(data[yr].swt_import) + parseInt(data[yr].swt_export),
                                    totalTradefm = '$' + numeral((totalTrade) * 0.000000001).format('0,0.0') + ' B',
                                    ...
                                    foreignTradefm = numeral((foreignTrade * 0.000001)).format('0,0.0') + ' M',
                                    ...
                                    //import indicators
                                    importPercent = (parseInt(data[yr].swt_import) / (parseInt(data[yr].swt_import) + parseInt(data[yr].swt_import))) * 100,
                                    importArray = [exportPercent, 100 - exportPercent],
                                    prevExport = (parseInt(data[yr - 1].swt_import) / (parseInt(data[yr - 1].swt_import) + parseInt(data[yr - 1].swt_import))) * 100,
                                    importChange = ((parseInt(data[yr].swt_import) - parseInt(data[yr - 1].swt_import)) / parseInt(data[yr - 1].swt_import)) * 100,
                                    usImportChange = ((parseInt(data[yr].us_import) - parseInt(data[yr - 1].us_import)) / parseInt(data[yr - 1].us_import)) * 100,
                                    importTons = parseInt(data[yr].swt_import) * 0.000000001,
                                    importComparison = calcDifference(exportChange, usExportChange),
                                    ...
                                    exportTons = parseInt(data[yr].swt_export) * 0.000000001,
                            ...
                            //update import indicators
                            $('#mi-import-change').html((importChange).toFixed(1) + '%');
                            $('#mi-import-percent').html(importPercent.toFixed(0) + '%');
                            $('#mi-import-national').html((usImportChange).toFixed(1) + '%');
                            $('#mi-import-value').html('$' + importTons.toFixed(1) + ' B');
                            $('#mi-import-indicator').html(calcChange(importChange));
                            ...
                            $('#mi-export-value').html('$' + exportTons.toFixed(1) + ' B');
                        ...
                        var newYear = miYear;
                        $('#landport-year-dropdown').on('change', 'input', function() {
                        ...
                        $(document).ready(function () {
                            $('#mi-vessel-count').html(numeral(955605).format('0,0'));
                        })

                        $(document).ready(function () {
                            $('#mi-vessel-percent').html(numeral(.08).format('0%'));
                        })

                        // $(document).ready(function () {
                        //     $('#mi-vessel-icon').addClass('glyphicon-circle-arrow-up');
                        // })
