$(document).ready(function () {
    ...
    d3.csv('data/d3/marineport_diagram.csv', function(port_points) {
        d3.csv('data/d3/marineport_activity.csv', function(port_data) {
            // d3.json("data/d3/river_diagram.js", function(json) {
            // d3.json("data/d3/Border.geojson", function(json) {
            // d3.json("data/d3/SDHarbor.geojson", function(json) {
                d3.json("data/d3/ocean_merge.json", function(json) {
                // d3.csv("data/d3/river_labels.csv", function(river_labels){
                d3.json("data/d3/PortSanDiego.json", function(river_labels){
                    ...
                    var scale = 11.12 / Math.max((bounds[1][0] - bounds[0][0]) / (port_dia_width), ((bounds[1][1] - bounds[0][1]) / port_dia_height));

                    var transl = [(port_dia_width - 3100 - scale * (bounds[1][0] + bounds[0][0])) / 2, (port_dia_height + 1350 - scale * (bounds[1][1] + bounds[0][1])) / 2];
                    ...
                    d3.csv("data/d3/maritimeIndicators.csv", function(data) {
                        ...
                        dataWindow.append('div')
                            .html('<div class="mi-dw-title">Trade for <span class="mi-dw-year">2016</span></div>' +
                                ...
                                'Containers: <span class="left_align"><span class="mi-dw-containers">xxx</span> k TEUs</span></div>');
                        ...
                        $(document).ready(function () {
                            $('#mi-vessel-count').html(444);
                        })

                        $(document).ready(function () {
                            $('#mi-vessel-percent').html(numeral(.05).format('0%'));
                        })

                        // // load port diagram on page load
                        // $(document).ready(function () {
                        //     $.getJSON("data/d3/PortSanDiego.json", function(river_labels){
                        //         $.each(river_labels, function(i, field){
                        //             $('#mi-port-diagram').append(field + " ");
                        //         });
                        //     });
                        // });
