    ...
    function load_region(){
        ...
        function buildCountyMap(){
            ...
            if ($('#c-region-map').length === 0) {
                ...
                d3.json('data/d3/Vicinity_20171109_v5.json', function(county5k) {
                    ...
                    counties.selectAll('path#c-san diego')
            ...
            for(var i=0; i<county_data.length; i++) {
                if (county_data[i].NAME === capitalizeFirstLetter(county)) {
                    var cdata = county_data[i],
                        county_ref = i;
                    if(parseInt(cdata.NAME) === 0){$('#c-stat-highway').hide();}else{$('#c-stat-highway').show();}
        ...
        function region_navigation(hash_elem){
            ...
            county = (hash_elem.length > 1) ? decodeURIComponent(hash_elem[1]) : 'none';
        ...
        function capitalizeFirstLetter(string) {
            if (county == "san diego") {
                // return string.charAt(0).toUpperCase() + string.slice(1, 3) + ' ' + string.charAt(4).toUpperCase() + string.slice(5);
        ...
        function buildMap(){
            ...
            if ($('#tradeMap').length === 0) {
                ...
                var scale_width = (bounds[1][0] - bounds[0][0] - 15) / (dt_width-50),
                    scale_height = (bounds[1][1] - bounds[0][1]) / 630,
                    scale_level = scale_width > scale_height ? (1000 / scale_width) : 1000,
